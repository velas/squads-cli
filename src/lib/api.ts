...
class API {
   ...
   constructor(
      ...
      this.squads = Squads.endpoint(connection.rpcEndpoint, wallet, {
         commitmentOrConfig: "confirmed",
         multisigProgramId: this.programId,
         programManagerProgramId: this.programManagerId,
      })
      this.wallet = wallet
   ...
   getTransactions = async (ms: any) => {
      ...
      const transactions = await Promise.all(
         [...new Array(txIndex)].map(async (_, i) => {
   ...
   getParsed = (
      account: anchor.web3.RpcResponseAndContext<anchor.web3.AccountInfo<
         Buffer | anchor.web3.ParsedAccountData
      > | null>,
   ) => {
      const { value } = account
   ...
   createTransferWithdrawAuthTx = async (
      msPDA: PublicKey,
   ...
   addKeyTransaction = async (msPDA: PublicKey, key: PublicKey) => {
      ...
      const [txInstructions, txPDA] = await (await txBuilder.withAddMember(key)).getInstructions()
      const activateIx = await this.squads.buildActivateTransaction(msPDA, txPDA)
