...
export const checkIsNFT = async (connection: Connection, acc: any) => {
   try {
      const edition = await programs.metadata.Metadata.getEdition(
         connection,
...
export const getNFTAccounts = async (connection: Connection, publicKey: PublicKey) => {
   ...
   return (
      Promise.all(tokenMap)
         ...
         .then(filtered => {
            return getMultipleAccountsBatch(
               connection,
               filtered.map(f => f.metadataPda),
            )
               .then(pdaRes => {
                  return pdaRes.map((mpr: any, i) => {
                     ...
                     if (
                        md &&
                        (md.data.data.creators ||
                           md.data.tokenStandard === 1 ||
                           filtered[i].amount.decimals === 0)
                     ) {
                        return {
         ...
         .then(async f => {
            for (const model of f) {
               if (model) {
                  try {
                     model.jsonData = await axios
                        .get(model.metadata.data.data.uri)
...
// can fit 250 ixes
export const createAuthorityUpdateTx = async (
   squadsSdk: Squads,
   multisig: PublicKey,
   ...
   if (txState.instructionIndex !== batchLength && attachFails.length > 0) {
      ws.write(
         `There were issues attaching some of the instructions, trying to attach them to tx ${txState.publicKey.toBase58()}\n`,
      )
      while (attachFails.length > 0) {
         ...
         const ix = updateMetadataAuthorityIx(
            newAuthority,
            currentAuthority,
...
export const getMetadataAccount = (mint: PublicKey) => {
   ...
   return PublicKey.findProgramAddressSync(
      [utils.bytes.utf8.encode("metadata"), METAPLEX_PROGRAM_ID.toBuffer(), mint.toBuffer()],
      METAPLEX_PROGRAM_ID,
   )[0]
}
...
export const validateMetadataAccount = async (
   connection: Connection,
...
export const checkMetadataAuthority = async (
   connection: Connection,
   metadataAccount: PublicKey,
   authority: PublicKey,
) => {
...
export const checkAllMetasAuthority = async (
   connection: Connection,
   mints: PublicKey[],
   authority: PublicKey,
) => {
...
export const sendTxMetaIx = async (
   msPDA: PublicKey,
   txPDA: PublicKey,
   member: PublicKey,
   dataObj: any,
   ...
   const trackMetaSig = anchor.utils.sha256.hash("global:track_meta")
   const ixDescrim = Buffer.from(trackMetaSig, "hex")
   const trackMetaData = Buffer.concat([ixDescrim.slice(0, 8)])
...
export const estimateBulkUpdate = async (
   sdk: Squads,
   connection: Connection,
   buckets: PublicKey[][],
   testKey: PublicKey,
...
export const estimateBulkWithdrawNFT = async (
   sdk: Squads,
   connection: Connection,
   buckets: PublicKey[][],
   testKey: PublicKey,
...
export const checkIfMintsAreValidAndOwnedByVault = async (
   connection: Connection,
   mints: PublicKey[],
   vault: PublicKey,
) => {
   ...
   try {
      ...
      const allTokens = await connection.getParsedTokenAccountsByOwner(vault, {
         programId: TOKEN_PROGRAM_ID,
      })
      for (const nft of loadedNFTs as any) {
         const index = loadedNFTs.indexOf(nft)
         if (!nft) failures.push(mints[index].toBase58())
         else {
            const found = allTokens.value
               .filter(value => value.account.data.parsed.info.tokenAmount.uiAmount > 0)
               .find(value => value.account.data.parsed.info.mint === nft?.mintAddress.toBase58())
            if (!found) failures.push(mints[index].toBase58())
            else success.push(nft?.mintAddress)
         }
      }
   } catch (e) {
      console.log(e)
...
export const createWithdrawNftTx = async (
   ...
   const metaplex = Metaplex.make(connection).use(walletAdapterIdentity(garbageWallet))
